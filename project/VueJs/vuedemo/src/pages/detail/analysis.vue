<template>
	<div class="sales-board">
		...
		<!--支付弹窗-->
		<bank-diaLog :isShow="isShowPayDialog"
			...
			<div class="button but-dialog-btn" @click="confirmBuy">
			确认购买
			</div>
		</bank-dialog>
...
import bankDiaLog from '../../components/base/Dialog.vue'
...
//			console.log(attr) 拿到的是数据类型（buyNum,buyType,versions,period）
//			console.log(val)  //拿到一个对象，是点击当前按钮，获取对应的{label:'',value:''},如果是多选，那么拿到的是一个数组对象
			this[attr] = val
			/*每次有数据更改的时候去重新发送一个数据请求*/
			this.getPrice()
		},
		//发送一个方法ajax，去拿价格
		getPrice(){
			/*先处理versions,转化为一个只有value的数组*/
			...
			this.$http.post('http://localhost:8081/api/getPrice', reqParams).then((res) => {
		...
		confirmBuy(){
			...
			let reqParams = {
				buyNum: this.buyNum,
				buyType: this.buyType.value,
				periods: this.period.value,
				versions: buyVersionArray.join(','),
				bankId:this.bankId
			}
			this.$http.post('http://localhost:8081/api/createOrder',reqParams).then((res)=>{
				/*返回订单的银行id的时候进行检查所选银行是否正确*/
